...
def do_full_experiment(parameters):
    ...
    outdir = "{}/{}/{}{}".format(parameters["outdir"], parameters["language"], random.randint(100,1000), get_signature(parameters["hyper"]))
    ...
    options="-F 1 -b 1 -t {train} -d {dev} -i {i} -f {tpl} -o {out} -N {hyper} {transitionsystem}".format(
                ...
                #######train = parameters["datadir"] + "/ftb/dev.ctbk", # for quick tests
                dev = parameters["datadir"] + "/{}/dev.ctbk".format(parameters["language"]),
                i = parameters["iterations"],
                tpl = tplfile,
                out = outdir,
                hyper = hyperparameters,
                transitionsystem = parameters["transitionsystem"])
        ...
        for model in models:
            ...
            for corpus in ["dev", "test"] :
                ...
                unix("perl eval07.pl -p -g {gold} -s  {pred}.conll -o {out}".format(
                        gold="{}/{}/{}.conll".format(parameters["datadir"], parameters["language"], corpus),
                        pred=pred_file,
                        out="{}/depeval_nopunct_{}_b{}_it{}".format(outdir, corpus, beam, i)))
        beam //= 2
...
def main() :
    ...
    parser.add_argument("--languages", type=str, nargs="+", default = ["dptb", "tiger_spmrl", "ftb", "negra"], help="List of languages")
    ...
    parser.add_argument("--embeddingsizefile", type=str, default=None, help="dir for files containing embedding size for each field (cat,tok,tag,morph1,...morphn")
    ...
    map_transition_systems = {"" : "", "gap" : "", "m0" : "-m 0", "m1" : "-m 1", "m2" : "-m 2", "m3" : "-m 3"}
    ...
    def get_params() :
        return {"datadir" : args.datadir,
                ...
                "transitionsystem" : map_transition_systems[args.transitionsystem]
                #"suffix" : args.suffix
                }
                                    ...
                                    for rnn_size in args.rnnsize :
                                        ...
                                        for charsize in args.charsize :
                                            for charbased_esize in args.charrnnsize :
                                                ...
                                                if args.embeddingsizefile :
                                                    embeddings = list(map(int, open(args.embeddingsizefile).read().strip().split()))
